...
export function FitnessPage() {
  ...
  const startWorkout = async () => {
    if (!template) return;
    await run(async () => {
      const workout = await api.create("workouts", { template_id: template.id, name: template.name, workout_date: localDate(), status: "in_progress", started_at: new Date().toISOString() });
...
function FitnessDialog({ dialog, close, template, run }: any) {
  ...
  const configs: Record<string, { title: string; collection: any; fields: FieldDefinition[]; defaults: any }> = {
    template: { title: "新建训练模板", collection: "workoutTemplates", fields: [{ name: "name", label: "模板名称", required: true }, { name: "weekday", label: "计划星期（1-7）", type: "number" }, { name: "notes", label: "说明", type: "textarea" }], defaults: {} },
    ...
    workout: { title: "安排一次训练", collection: "workouts", fields: [{ name: "name", label: "训练名称", required: true }, { name: "workout_date", label: "日期", type: "date", required: true }, { name: "feeling", label: "备注", type: "textarea" }], defaults: { template_id: template?.id, name: template?.name ?? "自主训练", workout_date: localDate(), status: "planned" } },
