...
My Own App 是 samuel的工作台 的开源项目：一款只在自己电脑上运行的工作与生活管理应用。它把计划、创作、开发、咨询、健身、饮食和娱乐放进同一个本地控制中心，不需要注册账号，也不依赖云数据库。
...
- 读书中心：书架、阅读进度统计、多次阅读笔记、封面与可选 PDF 内置阅读。
- 思考中心：每天一份结构化复盘、多条自由思考、来源分类、历史回顾和明日行动联动。
...
![samuel的工作台流光玻璃界面](docs/screenshots/liquid-glass.png)

### Notion 笔记

![samuel的工作台 Notion 笔记界面](docs/screenshots/notion-notebook.png)

### Neo-Brutalism

![samuel的工作台 Neo-Brutalism 界面](docs/screenshots/neo-brutalism.png)
...
只有安装依赖时需要联网。安装完成后，samuel的工作台的日常运行、保存、备份和恢复都不需要联网。
...
1. 检查 `127.0.0.1:4317` 上是否已有当前版本的 samuel的工作台。
2. 首次运行时生成本地生产构建。
